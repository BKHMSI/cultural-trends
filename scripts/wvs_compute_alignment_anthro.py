...
if __name__ == "__main__":
    ...
    result_config = []
    for ANTHRO in [True, False]:
        question_results = []
        result_config += [(model_country, LANG, MODEL, ANTHRO)]
        for qidx in selected_questions:
            ...
            if ANTHRO:
                # results_path = f"../dumps_wvs/q={qidx}_country={model_country}_lang={LANG}_model={MODEL}_anthro=True.csv"
                results_path = f"../dumps_wvs_2/q={qidx}_country={model_country}_lang={LANG}_model={MODEL}_anthro=True.csv"
            else:
    ...
    # breakpoint()
    for qidx in selected_questions:
        if qidx == skip_questions: continue
    ...
    for result_idx, result in enumerate(results):
        ...
        for qidx in tqdm(selected_questions):
            ...
            for persona in unique_personas:
                ...
                if len(other_survey_persona_row) == 0 and SKIP_SAME_ANS:
                    continue 
                ...
                try:
                    ...
                    if SKIP_SAME_ANS:
                        other_survey_answer = other_survey_persona_row[other_wvs_question_map[qidx]].values[0]

                        if survey_answer == other_survey_answer:
                            continue 
                    ...
                    for variant_idx, model_answer in enumerate(model_answers):
                        ...
                        if model_answer != -1:
                            ...
                            persona_json_results += [{
                                "question": qidx,
                                "variant": variant_idx,
                                "anthro-prompting": result_config[result_idx][-1],
                            ...
                            # score = (1-(np.abs(model_answer - survey_answer) / num_options)) * (model_answer not in wvs_response_map_reverse[str(qidx)] or wvs_response_map_reverse[str(qidx)][model_answer] not in ["Don't know", "No answer"])
                            # persona_results += [score]
                except:
                    breakpoint()
        ...
        print(f"MAE: {np.mean(mae_score)}")
        print(f"Accuracy: {np.mean(persona_results)}")

        final_exact = np.mean(mae_score) 
        final_random = np.mean(persona_random) 
        score = (final_exact - final_random) / (1 - final_random)
        print(f"Nael Score: {score}")
        print()

    write_json(f"persona_results_{SURVEY_COUNTRY}_anthro_{SKIP_SAME_ANS}.json", persona_json_results)
